...
To assess the capabilities of large language models (LLMs) beyond traditional fine-tuning, new benchmarks have been introduced.
...
\begin{table}[h!]
	...
	\label{tab:pal-math}
\end{table}
\textsc{PaL} is even more effective respect to other LLMs when tested on GSM-HARD dataset -- a version of GSM8K contains larger numbers (i.e., up to 7 digits).
...
In the second stage, the implementation phase, the plan generated in the first stage is used to guide the code generation.
...
Long-term memory is a crucial component in the planning process, allowing models to store and retrieve information from past experiences in addition to the short-term memory capabilities provided by in-context learning (ICL) in large language models (LLMs).
...
In particular, the generative agents leverage a memory stream mechanism for action planning and reflexion simulating human-like decision-behavior.
The memory stream is a long-term memory module that records, in natural language, a comprehensive list of the agent’s experiences.
...
Other studies~\cite{sun2023adaplanner, wang2023voyager} have also explored a use of memory called skill library mechanism to store successful plans, which can be reused and synthesized as complex plans for new tasks.
...
\end{figure}
To implement the long term memory, \textcite{wang2023voyager, wang2021milvus} propose tools like vector databases, which can be used to store plans or feedback into high-dimensional vectors.\footnote{A vector database is a type of database engineered specifically for handling vector data, which are arrays of numbers or embeddings representing various types of data objects.
...
\begin{figure}[h!]
	...
	\caption{Overview of MemoryBank. The memory storage stores past conversations, summarized events and user portraits, while the memory updating mechanism updates the memory storage. Memory retrieval recall relevant memory. Source: \textcite{zhong2023memorybankenhancinglargelanguage}}
