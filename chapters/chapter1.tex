...
\label{ch:large-language-models}
...
\begin{enumerate}
	\item \textbf{Statistical Language Models:} {These models were developed to capture the statistical properties of language, such as word frequencies and co-occurrences, to predict the likelihood of a given sequence of words based on the Markov assumption, which states that the probability of a word depends only on the previous \textit{n} words.
		      ...
		      However, these models are limited by the exponential number of transition probabilities to be estimated and the Markov assumption\footnote{The Markov assumption proposes that the future state of a process relies solely on the current state by disregarding the journey to the current state. Mathematically, it is expressed in terms the conditional probability \(P(S_t)\) -- the likelihood of an event occurring given the past states -- as \(P(S_{t+1} | S_t, S_{t-1} \ldots, S_1) = P(S_{t+1} | S_t)\), where \(S_t\) is the state at time \(t\). The Markov assumption simplifies the modelling process by reducing the number of parameters to estimate.}, which may not always hold true in the complexity of natural languages.
	...
	\item \textbf{Large Language Models (LLM):} {The emergence of large language models, characterised by their immense scale and complexity, has redefined the capabilities of language processing systems.
		      ...
		      \begin{figure}
			      \centering
			      \includegraphics[width=0.7\textwidth]{icl-example2}
			      ...
			      \label{fig:icl-example}
		      \end{figure}
		      Three typical examples of emergent abilities are:
		      \begin{enumerate}
			      \item \textbf{In-context learning:} {this ability has been formally observed in GPT-3, which is provided with a natural language instruction or task demonstrations; it can generate the expected output for test instances by completing the word sequence of the input text (as shown in Figure~\ref{fig:icl-example}). Importantly, this can be achieved without requiring additional training or gradient updates \footnote{\textcite{dai2022why} shows that in-context learning implicitly performs meta optimisation through the attention mechanism}.
...
Emergency is still a debated topic: \textcite{schaeffer2023emergentabilitieslargelanguage} shows that different metrics can reveal continuous improvement in LLM performance, challenging the concept of emergent abilities\footnote{Choosing a different metric it's possible to show that increasing the model size, leads to an improvement in correct sequences prediction in addition problems. Looking at these metrics, the "add" ability is not emergent but gradual and predictable.}, while others argue that the unpredictability of when and which metrics show abrupt improvement still supports the idea of emergence.
...
\subsection{\textsc{LLaMA}}
\label{subsec:llama}

\textsc{LLaMA}~\footnote{Large Language Model Meta AI} is a language model developed by Meta AI, designed to be a versatile and efficient foundation for a wide range of natural language processing (NLP) tasks.
\textsc{LLaMA} is built on a transformer architecture~\cite{vaswani2023attention}, similar to other large language models, with a range from 7B to 65B parameters.
Main differences between \textsc{LLaMA} and original Transformer architecture~\cite{vaswani2023attention} are the following:
\begin{enumerate}
	\item \textbf{Pre-normalization\footnote{Inspired by GTP-3 model}} {\textsc{LLaMA} uses pre-normalization\footnote{See Section \ref{subsubsec:normalization}}, which means that the normalization layer is placed before the self-attention and feed-forward layers.
	...
	\item \textbf{SwiGLU activation function\footnote{Inspired by PaLM model}} {\textsc{LLaMA} uses the SwiGLU\footnote{See Section \ref{subsubsec:activation-functions}} activation function by \textcite{shazeer2020glu}, which is a variant of the Gated Linear Unit (GLU) activation function.
	...
	\item \textbf{Rotary Embeddings\footnote{Inspired by GPTNeo model}} {\textsc{LLaMA} uses rotary embeddings by \textcite{su2021roformer}, which are a type of positional encoding that helps the model capture long-range dependencies in the input data.
	      }
\end{enumerate}
...
\begin{table}[ht]
	...
	\begin{tabular}{@{}ccccccccc@{}}
		\toprule
		Model                & params & dimension & \# heads & \# layers & learning               & batch & \# tokens & context \\
		                     &        &           &          &           & rate                   & size  &           & length  \\
		\midrule
		\textsc{LLaMA}       & 6.7B   & 4096      & 32       & 32        & \(3.0 \times 10^{-4}\) & 4M    & 1.0T      & 2k      \\
		\textsc{LLaMA}       & 13.0B  & 5120      & 40       & 40        & \(3.0 \times 10^{-4}\) & 4M    & 1.0T      & 2k      \\
		\textsc{LLaMA}       & 32.5B  & 6656      & 52       & 60        & \(1.5 \times 10^{-4}\) & 4M    & 1.4T      & 2k      \\
		\textsc{LLaMA}       & 65.2B  & 8192      & 64       & 80        & \(1.5 \times 10^{-4}\) & 4M    & 1.4T      & 2k      \\
		Code\textsc{LLaMA} 2 & 7B     & 4096      & 32       & 32        & $2.0 \times 10^{-4}$   & 4M    & 1.8T      & 16k     \\
		\textsc{LLaMA} 2     & 7B     & 4096      & 32       & 32        & $2.0 \times 10^{-4}$   & 4M    & 1.8T      & 4k      \\
		\textsc{LLaMA} 2     & 13B    & 5120      & 40       & 40        & $2.0 \times 10^{-4}$   & 4M    & 1.8T      & 4k      \\
		\textsc{LLaMA} 2     & 70B    & 8192      & 64       & 80        & $1.5 \times 10^{-4}$   & 4M    & 1.8T      & 4k      \\
		\textsc{LLaMA} 3     & 8B     & 4096      & 32       & 32        & $2.5 \times 10^{-4}$   & 4M    & 15T       & 8k      \\
		\textsc{LLaMA} 3     & 70B    & 8192      & 64       & 80        & $1.0 \times 10^{-4}$   & 4M    & 15T       & 8k      \\
		\textsc{LLaMA} 3.1   & 8B     & 4096      & 32       & 32        & $3.0 \times 10^{-4}$   & 4M    & 15T       & 128k    \\
		\textsc{LLaMA} 3.1   & 70B    & 8192      & 64       & 80        & $1.5 \times 10^{-4}$   & 4M    & 15T       & 128k    \\
		\textsc{LLaMA} 3.1   & 504B   & 16384     & 128      & 126       & $8.0 \times 10^{-5}$   & 4M    & 15T       & 128k    \\
		\bottomrule
	\end{tabular}
	\caption{\textsc{LLaMA} models sizes, architectures, and optimization hyper-parameters.
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth}{@{}lXXXXXXXXX@{}}
		...
		\addlinespace
		\textsc{LLaMA} & 7B     & 76.5  & 79.8 & 48.9 & 76.1      & 70.1       & 72.8  & 47.6  & 57.2 \\
		\textsc{LLaMA} & 13B    & 78.1  & 80.1 & 50.4 & 79.2      & 73.0       & 74.8  & 52.7  & 56.4 \\
		\textsc{LLaMA} & 33B    & 83.1  & 82.3 & 50.4 & 82.8      & 76.0       & 80.0  & 57.8  & 58.6 \\
		\textsc{LLaMA} & 65B    & 85.3  & 82.8 & 52.3 & 84.2      & 77.0       & 78.9  & 56.0  & 60.2 \\
		\bottomrule
	\end{tabularx}
...
The \textsc{LLaMA} models were trained exclusively on publicly available data, setting them apart from other models that rely on proprietary datasets~\footnote{Such as "Books --- 2TB" or "Social media conversations"}.
...
\textsc{LLaMA} models were designed with efficiency in mind, both in training and inference, allowing even the 13B parameter model to run on a single GPU\@.
A synthetic view of the \textsc{LLaMA} model family parameters is reported in Table~\ref{tab:llama-model-params}.
...
\subsubsection{\textsc{LLaMA} 2}
\label{subsubsec:llama-2}

\textsc{LLaMA} 2~\cite{touvron2023llama2} is a continuation of the \textsc{LLaMA} series, developed by Meta AI, released in scale from 7B to 70B parameters.
The pre-training data of the \textsc{LLaMA}2 model is a new mix of data from publicly available sources.
The training corpus is 40\% larger than the one used for \textsc{LLaMA} 1, and it is composed of a mix of text and a percentage of code data that is roughly 8\% of the total.
...
\begin{table}[h!]
	\centering
	\begin{tabularx}{0.5\textwidth}{XclcX}
		\hline
...
\textsc{LLaMA} 2 adopts most of the pretraining settings and model architecture from \textsc{LLaMA} 1, including the standard transformer architecture, pre-normalization using RMSNorm, the SwiGLU activation function, and rotary positional embeddings.
The optimizer used during the training is the same AdamW with the following hyper-parameters: $\beta_1=0.9, \beta_2=0.95, eps=10^{-5}$, a weight decay of 0.1, gradient clipping of 1.0, a cosine learning rate schedule and a warmup of 2000 steps.
The primary architectural differences from \textsc{LLaMA} 1 include increased context length and grouped-query attention (GQA).

\subsubsection{Code \textsc{LLaMA}}
\label{subsubsec:code-llama}

Code \textsc{LLaMA}~\cite{roziere2024codellamaopenfoundation} is a family of large language models for code generation based on \textsc{LLaMA} 2 providing infilling\footnote{With the terms infilling or code completion, we refer to the process of generating code snippets that complete a given code fragment.} capabilities, support
...
\begin{figure}[h!]
	...
	\caption{The Code \textsc{LLaMA} 70B specialization pipeline. The different fine-tuning stages are annotated with the number of tokens seen during training. Infilling-capable models are marked with the $\rightleftarrows$ symbol. Source: \textcite{roziere2024codellamaopenfoundation}.}
	\label{fig:code-llama}
\end{figure}

While most of the code generation models are trained on code only, Code \textsc{LLaMA} was fine-tuned starting from \textsc{LLaMA} 2, which was trained on general-purpose text and code data.
The comparison in \textcite{roziere2024codellamaopenfoundation} shows that initializing from \textsc{LLaMA} 2 leads to better performance on code generation tasks than initializing from a code-only model for a given budget as shown in Figure~\ref{fig:code-llama-comparison}.
Code \textsc{LLaMA} was fine-tuned on 500B extra tokens consisting mostly of code data (85\%).

\begin{figure}[ht!]
	\centering
	\includegraphics[width=0.7\textwidth]{codellama-loss}
	\caption{Comparison of Code \textsc{LLaMA} models versus an identical model trained from scratch. Source: \textcite{roziere2024codellamaopenfoundation}.}
	\label{fig:code-llama-comparison}
\end{figure}

\subsubsection{\textsc{LLaMA} 3}
\label{subsubsec:llama-3}

\textsc{LLaMA} 3~\cite{llama3} is a continuation of the \textsc{LLaMA} series, developed by Meta AI, with different model sizes: 8B, 70B, and 405B parameters.

\textsc{LLaMA} 3 uses a standard, dense Transformer architecture.
It does not deviate significantly from \textsc{LLaMA} and \textsc{LLaMA} 2 in terms of model architecture; therefore performance gains are primarily driven by improvements in data quality and diversity as well as by increased training scale.
Compared to \textsc{LLaMA} 2, \textsc{LLaMA} 3 has a few small changes in the model architecture:
\begin{enumerate}
	...
	\item a vocabulary with 128K tokens.
	      Compared to the \textsc{LLaMA} 2 tokenizer, the new tokenizer improves compression rates on a sample of English data from 3.17 to 3.94 characters per token.
	\item the RoPE base frequency hyper-parameter increased to 500,000 to support longer contexts.
\end{enumerate}
A summary of the key hyper-parameters of \textsc{LLaMA} 3 is shown in Table~\ref{tab:llama3-hyperparameters}.

\begin{table}[h!]
	\centering
	\caption{Overview of the key hyperparameters of \textsc{LLaMA} 3. We display settings for 8B, 70B, and 405B language models.}
	\begin{tabularx}{\textwidth}{Xccc}
		...
		\hline
	\end{tabularx}
	\caption{Overview of the key hyperparameters of \textsc{LLaMA} 3}
	\label{tab:llama3-hyperparameters}
\end{table}

The authors improved the quantity and quality of the data we used for pre-training and post-training compared to prior versions of \textsc{LLaMA}.
...
The pre-training corpus consists of about 15T tokens, which is about 50% larger than the one used for \textsc{LLaMA} 2.
...
In comparative benchmarks, Gemma models have demonstrated capabilities that exceed those of larger parameter models, such as \textsc{LLaMA} 2 (13B), indicating a significant efficiency in parameter utilization.
...
\begin{table}[!h]
	...
	\begin{tabularx}{\textwidth}{@{}lp{1.5cm}p{1cm}p{1cm}p{1.5cm}p{1.5cm}p{1.5cm}p{1cm}p{1cm}p{1cm}p{1cm}@{}}
		...
		FinMA~\cite{xie2023pixiu}              & \textsc{LLaMA}     & 7B, 30B & IFT, PE       & (G) 1T tokens                            & SA, TC, NER, QA          & FPB, FiQA-SA, Headline \newline FIN, FinQA, ConvFinQA & Y           & Y  & Y   \\
		InvestLM~\cite{yang2023investlm}       & \textsc{LLaMA}     & 65B     & PEFT          & (G) 1.4T tokens                          & SA, TC, SMP              & StockNet, CIKM18, BigData22                           & Y           & N  & N   \\
...
Within the four FinLLMs in Figure~\ref{tab:fin_llms}, FinMA~\cite{xie2023pixiu}, InvestLM~\cite{yang2023investlm}, and FinGPT~\cite{wang2023fingpt} are based on \textsc{LLaMA} or other open-source based models, while BloombergGPT~\cite{wu2023bloomberggpt} is a BLOOM-style closed-source model.
...
Here, studies have argued LLMs for human-like traits, encompassing self-perception, Theory of Mind (ToM)\footnote{The ability to impute unobservable mental states to others}, and emotional cognition~\cite{kosinski2023theoryofmind, amin2023affectivecomputing}.
...
Not surprisingly, initial excitement based on anecdotal performance of LLMs on reasoning tasks has been dissipated to some extent by the recent spate of studies, questioning the robustness of such behaviors–-be they planning~\cite{valmeekam2023planning, kambhampati2024llmscantplanhelp}, simple arithmetic and logic\cite{dziri2023faith}, theory of mind~\cite{ullman2023large, verma2024theory}, or general mathematical and abstract benchmarks~\cite{gendron2023large, mccoy2023embers}.
