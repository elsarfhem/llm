...
\label{subsec:definition-and-overview}
...
\begin{enumerate}
	...
	\item \textbf{Large Language Models (LLM):} {The emergence of large language models, characterised by their immense scale and complexity, has redefined the capabilities of language processing systems.
		      ...
		      \begin{enumerate}
			      ...
			      \item \textbf{Step-by-step reasoning:} { For small LMs, it is usually difficult to solve complex tasks that involve multiple reasoning steps (e.g., mathematical word problems).
				            ...
				            Recent studies have shown that size is not a deciding factor in the model's ability to perform step-by-step reasoning tasks. We will investigate this further in Section~\ref{subsec:cot-performance}.
			            }
		      \end{enumerate}
	      }
	\item \textbf{Small Language Models:} {
		      ...
		      \begin{enumerate}
			      \item Pruning: Eliminating redundant parameters from neural networks to simplify computations while preserving core performance.
			      \item Quantization: Representing model weights and activations in lower precision (e.g., 8-bit integers) to improve speed and reduce memory usage.
			      \item Low-Rank Factorization: Decomposing weight matrices into simpler approximations to lower computational demands.
			      \item Knowledge Distillation: Transferring knowledge from larger "teacher models" to smaller "student models", enabling compact versions to retain critical features.
		      \end{enumerate}
...
In the IR field, LLMs can be used to improve the performance of information retrieval systems through AI chatbots (i.e., ChatGPT), or integrating search engines like the New Bing \footnote{https://www.microsoft.com/it-it/bing?form=MA13FV} or using RAG\footnote{Retrieval-Augmented Generation}~\cite{gao2024retrievalaugmentedgenerationlargelanguage} pipelines.
...
We will delve into the emergent abilities of LLMs, such as in-context learning, instruction following, and step-by-step reasoning, and how these abilities can be leveraged to solve complex tasks in Section~\ref{ch:utilization}.
...
\label{subsec:scaling-law-in-large-language-models}
...
\subsection{Prominent Model Families}
\label{subsec:promiment-model-families}
...
OpenAI o1~\cite{openai2024reasoning} is a multimodal model developed by OpenAI, designed to process and generate text and images.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\textwidth]{o1-eval}
	\caption{o1 greatly improves over GPT-4o on challenging reasoning benchmarks. Solid bars show pass@1 accuracy and the shaded region shows the performance of majority vote (consensus) with 64 samples. Source: \textcite{openai2024reasoning}.}
	\label{fig:o1-eval}
\end{figure}

o1 has demonstrated proficiency in various domains, including advanced mathematics, coding, and scientific problem-solving, showcasing its versatility and potential for real-world applications as shown in Figure~\ref{fig:o1-eval}\footnote{OpenAI o1 ranks in the 89th percentile on competitive programming questions (Codeforces), places among the top 500 students in the US in a qualifier for the USA Math Olympiad (AIME), and exceeds human PhD-level accuracy on a benchmark of physics, biology, and chemistry problems (GPQA).}.
Datasets like MATH and GSM8K are no longer effective at differentiating models for recent frontier models\footnote{Claude 3.5 Sonnet and Gemini}.
...
This approach is a more robust way to teach human values and principles to model, improving performance on known jailbreaks\footnote{adversarial prompts that purposely try to circumvent model refusals for content it’s not supposed to produce} and safety benchmarks~\cite{openai2024openaio1card}.
...
While the models demonstrated strong reasoning and planning abilities, their overall performance in autonomy tasks did not surpass the best public model, \textit{Claude 3.5 Sonnet}.
The models struggled with tool usage and feedback responsiveness when placed in basic agent scaffolds\footnote{
...
In terms of their planning capabilities, \textcite{wang2024planningabilitiesopenaiso1} finds that the models excel at following constraints but face difficulties in decision-making and spatial reasoning.
The o1 model is evaluated from three key perspectives: feasibility\footnote{A plan must not only be executable but also ensure goal completion under real-world constraints.}, optimality\footnote{optimality concerns how efficiently the plan achieves its goal}, and generalizability\footnote{Whether a language model can successfully plan across a diverse range of scenarios, including those it may not have explicitly encountered during training.}.
...
As reported by New Scientist in \textcite{newscientist2024o3}, o3 also scored a record high of 75.7\% on the Abstraction and Reasoning Corpus (ARC) developed by Google software engineer François Chollet, a prestigious AI reasoning test, but did not yet complete the requirements for the "Grand Prize" requiring 85\% accuracy.
...
\begin{table}[ht]
	...
	\begin{tabular}{@{}ccccccccc@{}}
		...
		\midrule
		LLaMA       & 6.7B   & 4096      & 32       & 32        & \(3.0 \times 10^{-4}\) & 4M         & 1.0T      & 2k      \\
		LLaMA       & 13.0B  & 5120      & 40       & 40        & \(3.0 \times 10^{-4}\) & 4M         & 1.0T      & 2k      \\
		LLaMA       & 32.5B  & 6656      & 52       & 60        & \(1.5 \times 10^{-4}\) & 4M         & 1.4T      & 2k      \\
		LLaMA       & 65.2B  & 8192      & 64       & 80        & \(1.5 \times 10^{-4}\) & 4M         & 1.4T      & 2k      \\
		CodeLlama 2 & 7B     & 4096      & 32       & 32        & $2.0 \times 10^{-4}$   & 4M         & 1.8T      & 16k     \\
		LLaMA 2     & 7B     & 4096      & 32       & 32        & $2.0 \times 10^{-4}$   & 4M         & 1.8T      & 4k      \\
		LLaMA 2     & 13B    & 5120      & 40       & 40        & $2.0 \times 10^{-4}$   & 4M         & 1.8T      & 4k      \\
		LLaMA 2     & 70B    & 8192      & 64       & 80        & $1.5 \times 10^{-4}$   & 4M         & 1.8T      & 4k      \\
		LLaMA 3     & 8B     & 4096      & 32       & 32        & $2.5 \times 10^{-4}$   & 4M         & 15T       & 8k      \\
		LLaMA 3     & 70B    & 8192      & 64       & 80        & $1.0 \times 10^{-4}$   & 4M         & 15T       & 8k      \\
		LLaMA 3.1   & 8B     & 4096      & 32       & 32        & $3.0 \times 10^{-4}$   & 4M         & 15T       & 128k    \\
		LLaMA 3.1   & 70B    & 8192      & 64       & 80        & $1.5 \times 10^{-4}$   & 4M         & 15T       & 128k    \\
		LLaMA 3.1   & 504B   & 16384     & 128      & 126       & $8.0 \times 10^{-5}$   & 4M         & 15T       & 128k    \\
		\bottomrule
	\end{tabular}
	\caption{Llama models sizes, architectures, and optimization hyper-parameters.
...
\paragraph{Code Llama.}
\label{par:code-llama}
...
\begin{enumerate}
	\item improve inference speed and key-value caches during decoding by using grouped query attention (GQA) with 8 key-value heads
	...
	\item a vocabulary with 128K tokens. It improves compression rates on English data compared to the Llama 2 tokenizer.
...
\begin{table}[h!]
	...
	\begin{tabularx}{\textwidth}{Xccc}
		...
		\hline
	\end{tabularx}
	\caption{Overview of the key hyperparameters of Llama 3. Source: \textcite{llama3}.}
...
The dataset comprises approximately 50\% general knowledge tokens, 25\% mathematical and reasoning tokens, 17\% code tokens, and 8\% multilingual tokens~\cite{llama3}.
...
Claude models are a family of large language models developed by Anthropic, a research organization focused on building advanced AI systems~\cite{anthropic2024claude3}.
...
\begin{itemize}
	\item Parse complex, multi-step problems and provide structured solutions.
	\item Handle abstract reasoning with improved accuracy in scenarios involving nuanced logical patterns or uncommon use cases.
\end{itemize}
Comparing this version to its predecessors Claude 3 and Claude 3 Opus, the advancements in Claude 3.5 Sonnet are clear:
\begin{itemize}
	\item On reasoning benchmarks, Claude 3.5 Sonnet achieves higher accuracy, particularly in tests like GSM8K and MATH datasets.
	\item Interaction speeds are significantly faster, improving usability in real-time applications.
	\item Its coding capabilities surpass earlier versions in complexity and versatility, reflecting deeper training on software development datasets.
\end{itemize}

\subsection{Specialized Large Language Models}
\label{subsec:applications-of-large-language-models}
...
\subsubsection{LLMs in Healthcare}
\label{subsubsec:llms-in-healthcare}
...
\subsubsection{LLMs in Finance}
\label{subsubsec:llms-in-finance}
...
\begin{itemize}
	...
	\item \textbf{Headline:} {A dataset of financial news headlines, used for text classification \cite{sinha2021impact}. This dataset comprises 11,412 news headlines, where each headline is labelled with a binary classification (e.g., "price up" or "price down").}
...
\paragraph{BloombergGPT}
\label{par:bloomberggpt}
...
\label{subsubsec:llms-in-education}
...
\subsubsection{LLMs in Law}
\label{subsubsec:llms-in-law}
...
\subsubsection{LLMs in Scientific Research}
\label{subsubsec:llms-in-scientific-research}
...
It is unsurprising that the initial enthusiasm surrounding the anecdotal performance of LLMs on reasoning tasks has somewhat waned due to a wave of recent studies questioning the robustness of these abilities—whether in planning~\cite{valmeekam2023planning, kambhampati2024llmscantplanhelp}, basic arithmetic and logic~\cite{dziri2023faith}, theory of mind~\cite{ullman2023large, verma2024theory}, or broader mathematical and abstract benchmarks~\cite{gendron2023large, mccoy2023embers}.
