...
\section{Utilization Strategies and Techniques}
...
\subsubsection{ICL performance and origins}
...
\subsubsection{ICL future research}
...
\subsubsection{CoT performance and origins}
...
Studies have demonstrated that both patterns and text are crucial for CoT performance, as their removal can cause a significant decline in effectiveness: text enables LLMs to generate meaningful patterns, while patterns help LLMs comprehend tasks and produce text that facilitates their resolution~\cite{madaan2022text}.
...
We'll try to address this gap in the next section~\ref{ch:capabilities}, by conducting a series of experiments to evaluate the effectiveness of training on code data for reasoning tasks.
...
\subsection{Program-of-Thoughts}
\label{sec:program-of-thoughts}

PoT uses a programmatic approach to prompt LLMs to solve complex reasoning tasks proposed by \textcite{chen2022program}.
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{pot-cot}
...
As confirmed by our experiments in Section~\ref{ch:capabilities}, PoT can significantly improve performance on math problems compared to CoT\@.
...
\subsubsection{Commonsense knowledge}
...
\begin{itemize}
	\item	\textbf{General commonsense:} refers to knowledge that is widely shared and assumed to be true by most people, such as the sun rises in the east and sets in the west.
	\item	\textbf{Physical commonsense:} involves intuitive knowledge about the physical world, such as objects falling to the ground when dropped and water flowing downhill.
	\item	\textbf{Social commonsense:} involves knowledge about social norms, customs, and practices, such as it is polite to say "thank you" when making requests.
	\item	\textbf{Science commonsense:} involves knowledge about basic scientific principles, such as gravity pulling all objects on Earth to Earth’s centre.
	\item	\textbf{Event commonsense:} involves knowledge about the sequence of events and their causal relationships, such as if a glass is knocked over, the liquid inside will spill.
	\item	\textbf{Numerical commonsense:} involves knowledge about numbers, such as a human has two hands and ten fingers.
	\item	\textbf{Prototypical commonsense:} involves knowledge about typical or prototypical examples of concepts, such as a swallow is a kind of bird and a bird has wings.
	\item	\textbf{Temporal commonsense:} involves knowledge about time, such as travelling abroad requires a longer time than taking a walk.
\end{itemize}
...
Finally, the model's generalizability remains limited.
It struggles with tasks that require reasoning over unseen scenarios and symbolic reasoning, where action semantics diverge from natural language.
...
\subsubsection{Prompt and code based planning}
...
For instance, Faithful CoT~\cite{lyu2023faithful} and PAL~\cite{gao2022pal} divide a reasoning task into two stages: in the first stage, the LLM generates a plan based on the query, and in the second stage, a deterministic solver executes the plan to produce the final answer.
Additionally, similar code-based approaches can be employed in embedded agents, as demonstrated by methods like PROGPROMPT~\cite{singh2022progprompt} and LLM+P~\cite{liu2023llmp}.
...
When feedback data from the environment is not directly usable for plan refinement by LLMs, some approaches incorporate an explicit reasoning process to extract essential information from the feedback~\cite{chen2023chatcot, yao2022react}.
...
\subsubsection{LLM-modulo Framework}
...
\begin{figure}
	...
	\caption{LLMs serve as idea generators, while various external critics, each specializing in different aspects, evaluate and provide feedback on the proposed plan. Source: \textcite{kambhampati2024llmscantplanhelp}}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{rag-framework}
	\caption{Retrieval-Augmented Generation (RAG) Framework mainly consists of 3 steps. 1) Indexing. Documents are split into chunks, encoded into vectors, and stored in a vector database. 2) Retrieval. Retrieve the Top k chunks most relevant to the question based on semantic similarity. 3) Generation. Input the original question and the retrieved chunks together into LLM to generate the final answer. Source: \textcite{gao2024retrievalaugmentedgenerationlargelanguage}}
...
\begin{enumerate}
	...
	\item \textbf{Enhanced Knowledge Integration}: By querying external databases, RAG systems continuously update their knowledge base, addressing the limitations of static pre-trained models.

	\item \textbf{Improved Accuracy}: Retrieved data serves as contextual grounding, reducing hallucinations and increasing the factual reliability of generated outputs.

	\item \textbf{Domain Adaptability}: RAG enables LLMs to integrate domain-specific information, improving performance in specialized areas like law, medicine, and engineering.
...
\begin{enumerate}
	...
	\item \textbf{Naïve RAG}: it was the first iteration of RAG systems. It follows the traditional pipeline of indexing, retrieval, and generation, which is also characterized as a "Retrieve-Read" framework~\cite{ma2023queryrewritingretrievalaugmentedlarge}. This approach is simple and effective but suffers notable drawbacks in terms of retrieval precision (e.g., missing crucial information) and generation accuracy (e.g., allowing for hallucinations, toxicity or bias).

	\item \textbf{Advanced RAG}: it introduces specific improvements to address the limitations of Naïve RAG. About retrieval quality, it employes pre-retrieval and post-retrieval strategies to enhance the relevance of retrieved data. For indexing, it uses more sophisticated techniques like sliding window approach, fine-grained segmentation and metadata. It incorporates additional optimization techniques to streamline the retrieval process~\cite{ilin2023rag}.

	\item {
	      \textbf{Modular RAG}: this architecture advances beyond previous RAG paradigms (Naive and Advanced RAG) by offering greater adaptability, flexibility, and functionality. It introduces new components and interaction patterns to address the challenges of static and rigid retrieval-generation frameworks, making it suitable for diverse tasks and dynamic scenarios.
	      Modular RAG incorporates specialized modules to enhance retrieval and generation:

	      \begin{itemize}
		      \item Search Module: Supports direct searches across diverse data sources such as databases, search engines, and knowledge graphs using LLM-generated queries~\cite{ma2023query}.
		      \item RAGFusion: Implements multi-query strategies for diverse perspectives, utilizing parallel searches and re-ranking for knowledge discovery~\cite{shao2023enhancing}.
		      \item Memory Module: Uses LLM memory to iteratively align retrieval processes with data distribution and enable unbounded memory pools~\cite{cheng2023lift}.
		      \item Routing Module: Dynamically selects pathways (e.g., summarization or database querying) to ensure optimal information retrieval and merging~\cite{zheng2023take}.
		      \item Predict Module: Reduces redundancy and enhances context relevance by generating content directly via the LLM~\cite{yu2022generate}.
		      \item Task Adapter Module: Adapts RAG to downstream tasks, automating prompt retrieval for zero-shot scenarios and enabling task-specific retrievers through few-shot learning~\cite{wang2024bridging, asai2023selfrag}.
	      \end{itemize}
	      ...
	      \begin{itemize}
		      \item Rewrite-Retrieve-Read: Enhances retrieval queries through LLM-based query rewriting and feedback mechanisms, improving task performance~\cite{ma2023query}.
		      \item Generate-Read: Replaces retrieval with LLM-generated content for certain scenarios~\cite{yu2022generate}.
		      \item Recite-Read: Retrieves directly from model weights to better handle knowledge-intensive tasks \cite{cheng2023lift}.
		      \item Iterative and Hybrid Retrieval: Combines multiple retrieval strategies, including keyword, semantic, and vector searches, or uses hypothetical document embeddings (HyDE) for improved relevance~\cite{shao2023enhancing}.
		      \item Dynamic Frameworks: Frameworks like DSP~\cite{zheng2023take} and ITERRETGEN~\cite{shao2023enhancing} iteratively process retrieval and reading steps, leveraging module outputs to enhance system performance.
	      \end{itemize}
	      ...
	      Techniques like FLARE \cite{asai2023selfrag} dynamically assess the necessity of retrieval in a given context.
	      Additionally, the architecture supports integration with technologies such as fine-tuning (e.g., retriever or generator optimization), reinforcement learning, and collaborative fine-tuning \cite{wang2024bridging, asai2023selfrag}.
	      }
...
\begin{enumerate}
	...
	\item {
	      \textbf{Generation}: RAG systems benefit significantly from post-retrieval adjustments to both the retrieved content and the underlying language models (LLMs). Directly feeding raw, retrieved data into an LLM is suboptimal, as redundant or overly lengthy contexts can dilute the quality of the final output.
	      ...
	      \begin{itemize}
		      \item Token Filtering: Small Language Models (SLMs) such as GPT-2 Small are used to remove less critical tokens while maintaining semantic integrity.
		      \item Information Extractors: PRCA trains specialized models to extract relevant content, while RECOMP uses contrastive learning to train condensers for refining context~\cite{cheng2023lift}, \cite{wang2024bridging}.
		      \item Filter-Reranker Paradigm: Combines SLMs as filters and LLMs as rerankers to improve downstream information extraction tasks. For example, Chatlaw incorporates LLM critiques to assess and filter legal provisions based on relevance~\cite{ma2023query}.
	      \end{itemize}


	      \begin{itemize}
		      \item Rule-Based Methods: These rely on predefined metrics like diversity, relevance, or Mean Reciprocal Rank (MRR).
		      \item Model-Based Approaches: Encoder-decoder models such as SpanBERT or specialized rerankers like Cohere or GPT-based reranking mechanisms reorder documents effectively~\cite{yu2022generate, zheng2023take}.
	      \end{itemize}
	      ...
	      \begin{itemize}
		      \item
		            ...
		            Fine-tuning LLMs on specialized datasets improves their adaptability to unique data formats or stylistic requirements. Frameworks like SANTA leverage contrastive learning for retriever training and reinforcement learning to align outputs with human preferences \cite{shao2023enhancing, cheng2023lift}.}
		      \item { Distillation:
		      ...
		      \item { Alignment Techniques:
		            Fine-tuning aligns retriever and generator preferences. For instance, RA-DIT uses KL divergence to align scoring functions between the retriever and the generator, enhancing overall coherence in retrieval-generation workflows \cite{asai2023selfrag}. }
	      \end{itemize}
	      }

	\item {\textbf{Augmentation}: the standard practice involves a single retrieval step followed by a generative output. While effective for straightforward tasks, this approach is often insufficient for more complex problems requiring multi-step reasoning, as it limits the scope of retrieved information \cite{yoran2023making}. To address these limitations, various iterative, recursive, and adaptive retrieval strategies have been proposed, enabling RAG systems to dynamically enhance their retrieval and generation processes.
	      ...
	      ITERRETGEN \cite{shao2023enhancing} exemplifies this approach by combining "retrieval-enhanced generation" with "generation-enhanced retrieval." It iteratively refines the context, ensuring that the knowledge retrieved aligns closely with the specific task at hand. This synergy facilitates the generation of more accurate and contextually relevant responses in subsequent iterations.
	      ...
	      IRCoT \cite{trivedi2022interleaving} employs a chain-of-thought (CoT) approach, using retrieval results to iteratively refine the CoT reasoning process. ToC (Tree of Clarifications) \cite{kim2023tree} systematically addresses ambiguities in queries by constructing clarification trees that refine the retrieval process step-by-step.
	      Recursive retrieval often pairs with multi-hop retrieval for graph-structured data, extracting interconnected knowledge. This combination is particularly effective for hierarchical or multi-document environments, where summaries or structured indices aid in refining subsequent retrieval steps \cite{li2023chain}.
	      ...
	      Flare \cite{asai2023selfrag} and Self-RAG \cite{cheng2023lift} enable LLMs to determine optimal retrieval moments and content, improving the adaptive capabilities of RAG frameworks.
	      GraphToolformer \cite{li2023chain} divides retrieval into distinct stages, where LLMs actively utilize tools such as retrievers and apply techniques like Self-Ask or few-shot prompts to guide the process.
	      WebGPT \cite{nakano2021webgpt} integrates reinforcement learning to train LLMs for autonomous search engine usage. By leveraging special tokens for actions such as querying, browsing, and citing sources, it mimics an agent actively gathering and validating information during generation.
	      }
...
\begin{itemize}
	...
	\item Retrieval Precision: Measures the relevance of retrieved data.

	\item Generation Accuracy: Assesses the factual correctness of outputs.

	\item End-to-End Performance: Evaluates the overall coherence, fluency, and informativeness of the system.

\end{itemize}

Benchmarks such as SQuAD~\cite{rajpurkar2016squad100000questionsmachine}, Natural Questions~\cite{kwiatkowski-etal-2019-natural}, and specialized datasets for retrieval tasks are widely used for assessment.

\noindent Despite its promise, RAG faces several challenges:

\begin{enumerate}

	\item Retrieval Latency: Efficiently querying large databases in real time remains a technical hurdle.

	\item Data Quality: The reliability of generated outputs depends heavily on the quality of retrieved data.

	\item Scalability: Handling large-scale retrieval tasks while maintaining high generation quality is complex.

\end{enumerate}

\noindent Future research avenues include:

\begin{itemize}

	\item Expanding RAG frameworks to support multi-modal inputs, such as text, images, and audio.

	\item Enhancing retrieval efficiency through novel indexing and search techniques.

	\item Improving integration mechanisms for tighter coupling between retrieval and generation modules.
