...
We will first provide an in-depth survey on LLMs, beginning with a foundational overview of their development, pre-training strategies, and architectural variations.
...
We will also investigate their application across various domains, such as healthcare, finance, education, law, and scientific research in Section~\ref{sec:large-language-models}.
...
The second objective of the present paper is to explore some of the mechanisms that enable LLMs to perform tasks previously deemed impossible for machine learning systems.
In particular, we will try to address a number of fundamental questions.
...
\noindent Key research questions guiding this paper include:
...
%This work will investigate the main components, strategies, and techniques that underpin the development of LLMs in literature, shedding light on how these models can be leveraged to solve complex tasks in Section \ref{ch:utilization}.
...
This work investigates LLMs by examining their foundational architecture, the training processes that enable them to handle diverse tasks, and the strategies that improve their capabilities.
...
Then we focused on the CoT and PoT abilities of LLMs to understand the origin of these abilities in Section~\ref{ch:capabilities}
